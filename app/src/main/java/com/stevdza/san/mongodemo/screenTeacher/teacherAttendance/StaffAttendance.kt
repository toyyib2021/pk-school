...
fun StaffAttendanceHistory(
    ...
    var term by remember { mutableStateOf(currentTerm.value) }
    ...
    val attendanceListForAStaff =  staffDashboardVM.teacherAttendanceData.value.filter {
        it.term == term && it.session == currentSessions
                && it.staffId == staffId}
    ...
    Column(modifier = Modifier
        ...
        Column(modifier = Modifier
            ...
            TopBarWithDownload(onBackClick = { onBackClick(post) }, title = currentSessions, onDownloadClick = {
                if (attendanceListForAStaff.isNotEmpty()){
                    Intent(Intent.ACTION_CREATE_DOCUMENT)
                        .apply {
                            ...
                            putExtra(
                                Intent.EXTRA_TITLE,
                                "$staffName Attendance for $term.pdf"
                            )
                        }
                        .also {
        ...
        LazyColumn(modifier = Modifier
            ...
            items(attendanceListForAStaff.subList(startIndex, endIndex).reversed()){
                StaffAttendanceList(name = it.name, position = it.position, date = it.date,
                    timeIn = it.timeIn, timeOut = it.timeOut,
                    hourIn = hourIn.toInt(),
                    hourOut =  hourOut.toInt(),
                    minIn = minIn.toInt(),
                    minOut =minOut.toInt()
